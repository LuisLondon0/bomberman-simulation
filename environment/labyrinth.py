...
class LabyrinthModel(Model):
    def __init__(self, width, height, map, search_strategy, heuristic, difficulty, depth):
        super().__init__()
        self.grid = MultiGrid(width, height, True)
        self.schedule = RandomActivation(self)
        self.goal_coords = None

        search_strategy = SearchFactory.create_search(search_strategy, heuristic, self)
        ...
        for y, row in enumerate(map):
            for x, cell in enumerate(row):
                if cell == "C":
                    ...
                    bomberman = AgentFactory.create_agent("bomberman", (x, y), self, search_strategy = search_strategy, depth = depth)
                    ...
                    self.grid.place_agent(road, (x, y))
                    self.schedule.add(road)
                    goal = AgentFactory.create_agent("enemy", (x, y), self, difficulty = difficulty)
                    self.grid.place_agent(goal, (x, y))
                    self.schedule.add(goal)
    ...
    def getMatrix(self):
        grid_structure = [[None for _ in range(self.grid.width)] for _ in range(self.grid.height)]

        for x in range(self.grid.width):
            for y in range(self.grid.height):
                cellmates = [agent.__class__ for agent in self.grid.get_cell_list_contents((x, y))]
                grid_structure[y][x] = cellmates
